...
contract( "TrustedList", accounts => {
	...
	before( async () => {
		const pkd = await PKD.new();
		const trustedList = await TrustedList.new( pkd.address, "0x201" );
		const subTrustedList = await TrustedList.new( trustedList.address, "0x203" );

		await subTrustedList.register( accounts[1], `did:lac:main:${accounts[1]}`, now + 1000 );
		await subTrustedList.register( accounts[2], `did:lac:main:${accounts[2]}`, now + 1000 );

		await trustedList.register( subTrustedList.address, `did:lac:main:${accounts[3]}`, now + 1000 );
	} );
